...
export default function SalesNotification() {
  ...
  useEffect(() => {
    const showNotification = () => {
      ...
      const randomName =
        sriLankanNames[Math.floor(Math.random() * sriLankanNames.length)];
      const randomProduct =
  ...
  return (
    <div
      className={`fixed bottom-4 left-4 md:bottom-6 md:left-6 z-[200] max-w-[200px] md:max-w-[240px] w-full transition-all duration-700 ease-in-out transform ${
        isVisible
      ...
      <div
        className={`absolute -inset-0.5 bg-gold/10 blur-sm -z-10 transition-opacity duration-1000 rounded-lg ${
